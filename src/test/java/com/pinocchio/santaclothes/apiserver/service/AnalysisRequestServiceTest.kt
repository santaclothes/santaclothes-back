...
class AnalysisRequestServiceTest(
    ...
    @Test
    fun toReported(@Autowired analysisRequestRepository: AnalysisRequestRepository) {
        // given
        val analysisRequestId = analysisRequestRepository.save(
            AnalysisRequest(
                userToken = "userToken",
                status = AnalysisStatus.CLASSIFIED,
                cloth = Cloth(
        ...
        // when
        sut.withStatus(analysisRequestId, AnalysisStatus.REPORTED)

        val actual = analysisRequestRepository.findById(analysisRequestId).orElseThrow()
        then(actual.cloth.careLabel).isNull()
    }
}
